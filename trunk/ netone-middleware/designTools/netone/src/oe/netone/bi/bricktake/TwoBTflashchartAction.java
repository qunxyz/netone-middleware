 ...
 *  
 */
public class TwoBTflashchartAction extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			upof1= resource.loadResourceByNatural(naturalname);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}	
	    ...
	    String strname=cvList.get(0).getId();
	   ...
	   //通过naturalname 来获取资源里面的数据
			...
			try {
				upof = resourceRmi.loadResourceByNatural(strname);
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}	
...
//		data.setYAxisLabel(yString);//
    	...
    	data.setParams("{\"name1\":\""+strname+"\",\"name2\":\""+naturalname+"\"}");
		...
		String json = service.entry(ChartCompVar.COMP_LINE_2L_CHART, data);
		// 3.输出参数
		service.output(request, json, data.getTitle());
		}
		if(OGXML.getTubiaotype().equals("zhutu")){
			// 2.指定输出报表格式 具体变量查看com.report.flashchart.entity.ChartCompVar.java
			String json = service.entry(ChartCompVar.COMP_BAR_2L_CHART, data);
			// 3.输出参数
			service.output(request, json, data.getTitle());
		}
		if(OGXML.getTubiaotype().equals("bingtu")){
			 //2.指定输出报表格式 具体变量查看com.report.flashchart.entity.ChartCompVar.java
			String json = service.entry(ChartCompVar.COMP_PIE_2L_CHART, data);
			// 3.输出参数
			service.output(request, json, data.getTitle());
		}
