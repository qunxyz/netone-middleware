...
<script type="text/javascript">
  Ext.ns('Buss.Data');
    		 ...
    		 initComponent: function() {
    			...
    			var config = {
    				...
    				store: new Ext.data.Store({
    					url: "<c:url value='/dic.do?method=onList' />", //JSON数据
    				...
    				columns:[
    						...
    						{header: "编码",width:100,dataIndex: 'code',sortable: true},
      ...
      Ext.onReady(function(){//页面加载时候触发事件
       ...
       function _add(){
       		openModalDialogWindow('新增','<c:url value="/dic.do?method=onEditView" />'+'&type=${param.type}' ,300,160); 
       }
       function _edit(){
            ...
            var url = '<c:url value="/dic.do?method=onEditView"/>'+ '&sid=' + guid +'&type=${param.type}' ;//*
   	      	  ...
   	      	  Ext.MessageBox.confirm("系统提示",'您确定要删除所选数据吗？',function(btnId){
   				    ...
   				    var str = Ext.util.Format.substr(str,0,str.length - 1);
   						Ext.Ajax.request({//AJAX删除
   								url : '<c:url value="/dic.do?method=onDelete" />',
   								params : {sid:str},
   								method : 'POST',
