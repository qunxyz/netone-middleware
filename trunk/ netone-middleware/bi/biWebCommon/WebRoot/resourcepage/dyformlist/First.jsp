...
<HTML dir="LTR" lang="zh-CN">
	<HEAD>
		...
		function nextsubmit(){
			...
			}else if(document.getElementsByName("selecttype")[1].checked){
			   window.location.href=document.getElementsByName("selecttype")[1].value;
			}else{
				if(document.all.nal.value==""){
					alert("输入的名字不允许为空!");
					return false;
				}
			   window.location.href=document.getElementsByName("selecttype")[2].value+"&naturalname="+document.all.nal.value;
			}
	...
	<BODY bgcolor="#FFFFFF">
		<FORM ACTION="" METHOD="GET" name="additem">
			...
			<DIV
				...
				<hr>
				<br>
				<FONT CLASS="OecInstructionText">帮助提示:请选择一种表单设计模式（动态表单是设计新表单，SQL表单是基于已有的库表来创建表单。两种模式的最终结果将映射成一张库表与描述文件）
					然后单击“下一步”。</FONT>
				<!-- End --Title----功能标题与提示---->
				<BR>
				...
				<DIV
					...
					<input type='radio' name='selecttype'
						value='<%=baseRootPath%>dyForm/design/system/form/createview.do?pagepath=${param.pagepath}'
						checked>
					动态表单
					<br>
					<br>
					<input type='radio' name='selecttype'
						value='<%=basePath%>NextSummary.do?task=First&pagepath=${param.pagepath}'>
					SQL表单
					<br>
					<br>
					<input type='radio' name='selecttype'
						value='<%=basePath%>CopyBussFormSvl?pagepath=${param.pagepath}&id=${param.id}'>
					复制SQL表单
