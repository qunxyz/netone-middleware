					...
					<table class="main" width="500px" style="margin-top: 20px">
						...
						<tr>
							<td class="labeltitle_td">
								<input type="file" id="importFile" name="importFile" style="height: 23px;" />
								&nbsp;&nbsp;
								<input type="button" onclick="onSaveButton()" value="确 定" class="btn">
								&nbsp;&nbsp; &nbsp;&nbsp;
								<input type="button" onclick="dowanLoad()" value="模版下载" class="btn">
							</td>
						</tr>
					</table>
				</form>
...
<script>
   function onSaveButton(){
	  ...
	  if(importFileObj.value == ''){
	      Ext.MessageBox.alert('系统提示','请选择要导入的分销商信息文件！');
		 return  ;
	  }
	  var arrayList=importFileObj.value.split(".");
	   if(arrayList[1]!='xls'){
	    ...
	    Ext.Ajax.request({
					url :'<c:url value="/client/import.do?method=onImportChildClientFile"/>',//请求的服务器地址
   ...
   function dowanLoad(){
       openWinCenter('下载','<c:url value="/client/import.do?method=onDownLoadClientFile"/>&type=child', 500,250,true);
   } 
