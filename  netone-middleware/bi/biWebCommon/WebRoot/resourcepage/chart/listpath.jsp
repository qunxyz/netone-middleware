				...
				<table width="96%" border="0" align="center" cellpadding="5"
					...
					<tr>
						<td colspan='9' align='right' nowrap>
							<script type="text/javascript">
								function newChart(){
									window.open("<%=path%>/NextChart.do?task=First&pagepath=${pathreal}",'_blank');
								}
								function editChart(value){

									window.open('<%=path%>/NextChart.do?task=Modify&chkid='+value,'_blank');
								}
								//原始图表页面
								function toChart(value){
									window.open("<%=basePath%>flowpage.do?chkid="+value+"&act=table&task=chart0");
								}	
							</script>
							&nbsp;&nbsp;
							<input type="button" value="新建图表" onclick="newChart();"
								class="butt">
							&nbsp;&nbsp;
					...
					<c:forEach items="${list}" var="list">
						<tr>
							...
							<td nowrap>
								...
								<a href="javascript:test('${list.id}');">[图表分析X]</a>
								<a href="javascript:toChart('${list.id}');">[图表分析]</a>
								<a href="javascript:editChart('${list.id}');">[修改条件]</a>
								<a href="javascript:edit('${list.id}');">修改</a>
								<a href="javascript:del('${list.id}');">删除</a>
								<a href="javascript:checkthis('${list.id}','${list.naturalname}');">选择</a>
							</td>
						</tr>
					</c:forEach>
				</table>
