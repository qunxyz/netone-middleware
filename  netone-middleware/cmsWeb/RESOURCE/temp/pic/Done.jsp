...
<%
	...
	String script = "$:PIC{id:=" + picid + "&+url:=" + linkInfo
...
<HTML>
	<HEAD>
		...
		//上一步脚本
		function forward(){
			window.location.href='Step.jsp?width=<%=width%>&height=<%=height%>&linkType=<%=linkType%>&linkvalue=<%=linkvalue%>&parentid=<%=parentid%>&pagepath=<%=pagepath%>&linkvaluepage=<%=linkvaluepage%>&picid=<%=picid%>';
			...
			<DIV
				...
				<DIV
					...
					<TABLE BORDER="0">
						...
						<tr>
							<td>
								<iframe src='' name='preivewpic' style='width:500;height:500'></iframe>
