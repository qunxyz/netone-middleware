...
public class PDPRolePermission implements PDP {
	...
	public ResponseCtx evaluate(String code, RequestCtx request) throws Exception {
		...
		for (Iterator iter = resourcectx.getAttribute().iterator(); iter.hasNext();) {
			...
			String roleid = permissionManager.getRoleIdFromNaturalName(participant);
			...
			if ("*".equals(lastSymbol)) {
				...
				for (Iterator iteror = list.iterator(); iteror.hasNext();) {
					...
					if (!permissionService.checkRolePermission(roleid, uproobj.getOu(), actionvalue)) {
						statusvalue = "0";
						break;
					}
				}
			} else {
				if (permissionService.checkRolePermission(roleid, dnname, actionvalue)) {
					statusvalue = "1";
				} else {
					statusvalue = "0";
				}
