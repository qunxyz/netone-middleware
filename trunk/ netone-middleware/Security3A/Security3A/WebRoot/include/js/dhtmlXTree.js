...
dhtmlXTreeObject.prototype.disableCheckbox = function(itemId,mode){
	...
	mode = convertStringToBoolean(mode);
	this._disableCheckbox(sNode,mode);
};
//hls
dhtmlXTreeObject.prototype._disableCheckbox = function(sNode,mode){
	var z = sNode.htmlNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0];
	if(mode){
		if(!sNode.dscheck){
			sNode.dscheck = sNode.checkstate + 3 ;
			this._setSrc(z, this.imPath + ((sNode.parentObject._r_logic || this._frbtr) ? this.radioArray : this.checkArray)[sNode.dscheck]);
		}
	}else{
		if(sNode.dscheck != null){
			sNode.dscheck = null ;
			this._setSrc(z, this.imPath + ((sNode.parentObject._r_logic || this._frbtr) ? this.radioArray : this.checkArray)[sNode.checkstate]);
		}
	}
