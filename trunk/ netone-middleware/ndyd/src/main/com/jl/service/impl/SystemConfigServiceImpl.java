...
import com.jl.entity.SystemConfig;
import com.jl.service.BaseService;
import com.jl.service.SystemConfigService;

public class SystemConfigServiceImpl extends BaseService implements
		SystemConfigService {
	/** 日志 */
	...
	public void deleteInfo(HttpServletRequest request,
		...
		try {
			...
			this.commonDAO.deleteBatch("SystemConfig.deleteSystemConfig", sysConfigList);
			json.put("tip", "删除信息成功!");
		} catch (Exception e) {
		...
		try {
			if (StringUtils.isNotEmpty(sid)) {
				request.setAttribute("sysconfig", this.commonDAO.findForObject(
						"SystemConfig.loadSystemConfig", sid));
			}
		} catch (Exception e) {
			LOG.error("加载失败！", e);
		}
	...
	public void queryInfo(HttpServletRequest request,
		...
		try {
			obj = this.commonDAO.selectForPage("SystemConfig.totalPageByCondition",
					"SystemConfig.resultPageByCondition", conditionMap, obj);
	...
	public void saveOrUpdate(HttpServletRequest request,
			HttpServletResponse response) {
		JSONObject json = new JSONObject();
		SystemConfig config = this.getSystemConfig(request);// 提交参数包装
		try {
			if (StringUtils.isNotEmpty(config.getSid())) {
				this.commonDAO.update("SystemConfig.updateSystemConfig", config);
			} else {
				config = (SystemConfig) this.commonDAO.insert(
						"SystemConfig.insertSystemConfig", config);
			}
			json.put("tip", "保存信息成功!");
		} catch (Exception e) {
	...
	public void loadSystemConfigToSelect(HttpServletRequest request,
		...
		try {
			coll = (Collection) this.commonDAO.select("SystemConfig.loadSystemConfigToSelect", conditionMap);
