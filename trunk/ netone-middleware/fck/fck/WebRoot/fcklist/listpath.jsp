...
<html>
	...
	//单选择 需要根据应用的需要来订制
		function checkthis(id,naturalname,inclusion,parent,name) {
		if(opener.selecthis==undefined){
			alert('非选择状态');
			return;
		}
			opener.selecthis(id,naturalname,inclusion,parent,name);
			window.close();
		}
		//多选择 需要根据应用的需要来订制
		function checkthese(){
					...
					<c:forEach items="${list}" var="list">
						<tr>
							...
							<td nowrap>
								...
								<a href="javascript:view1('${list.id}');">浏览</a>
								</c:if>
								<a href="javascript:del('${list.id}');">删除</a>
								<c:if test="${list.inclusion == '0'}">
								<a href="javascript:checkthis('${list.id}','${list.name}','${list.inclusion}','${list.parentdir}','${list.name}');">选择</a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
