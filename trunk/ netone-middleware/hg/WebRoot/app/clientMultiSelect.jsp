		 ...
		 initComponent: function() {
			...
			var config = {
				...
				store : new Ext.data.Store({
							url : "<c:url value='/app.do?method=onFindPartTree' />", // JSON数据
				...
				columns : [index,
					cb, 
					{header: "编码", align:'center',dataIndex: 'partCode',sortable : true,
						renderer : function (value, metadata, record, rowIndex, columnIndex, store) {   
						    //build the qtip:   
						    var title = '&nbsp;' +record.get('partCode')+' ' + record.get('partName');   
					...
					{header: "名称", align:'center',dataIndex: 'partName',sortable : true}
				],
	      ...
	      this.store.on('beforeload', function(store,options){
				  ...
				  options.params.node = Ext.get('node').dom.value;
				  options.params.partCode = Ext.get('partCode').dom.value;
				  return true;
		  });
							...
							var l = "";
						     ...
						     if (len<=0) {return;}
						      for ( var g = 0; g < len; g++) {
						       ...
						       l += f.getAt(g).data.partId;
						       j += f.getAt(g).data.partCode;
						       k += f.getAt(g).data.partName;
						      }
						     _select(l,j,k);
						}},
				        ...
				        tbar:new Ext.Toolbar([
							{
								xtype:'panel',
								baseCls : 'x-plain',
								html:'<input id=\'partCode\' type=\'text\' onkeyup=\'onKeyUpEvent(event);\'>'
							},
							'-',
...
function add_all(){//添加
	...
	for ( var p = 0; p < x.length; p++) {
	 var q = x[p].data.partId;
	 var v = x[p].data.partCode;
	 var k = x[p].data.partName;
	 var s = false;
	 for ( var o = 0; o < u.getCount(); o++) {
	  if (u.getAt(o).data.partId == q) {
	   s = true;
	   break;
	  }
	 }
...
function _select(ids,values,texts){
	if (window.opener && window.opener.location){
		opener.$selectUser(ids,values,texts);window.close();
	} else {
		alert('找不到父窗口!')
	}
}
