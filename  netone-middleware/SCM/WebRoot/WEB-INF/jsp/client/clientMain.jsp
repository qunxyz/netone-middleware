		 ...
		 initComponent: function() {
			...
			var config = {
				...
				store: new Ext.data.Store({
					url: "<c:url value='/client/client.do?method=onList' />"+'&flag=client', //JSON数据
				...
				columns:[
						...
						{header: "公司", width: 120, dataIndex: 'company', sortable: true,hidden: true}
						],
  ...
  Client.Layout.Viewport =  Ext.extend(Ext.Viewport, {
	       ...
	       var config = {
				 ...
				 items:[{
					 ...
					 items:[
					  ...
					  ,'-',new Ext.Toolbar.Button({
								  text:'客户类型',
								  id: 'ext_config_clientType_set',
								  iconCls: "addIcon"
					  }),'-',new Ext.Toolbar.Button({
								  text:'市场类型',
								  id: 'ext_config_marketType_set',
								  iconCls: "addIcon"
					  })
                     ]
                	},{
   ...
   Ext.onReady(function(){//页面加载时候触发事件
	...
	Ext.EventManager.addListener(Ext.get('ext_b_add'), 'click', function(){//新增
			openWinCenter('新增','<c:url value='/client/client.do?method=onEditView' />', 600,400,true);
	});
	
	
	//--------------------------------------新增客户类型---------
	 Ext.EventManager.addListener(Ext.get('ext_config_clientType_set'), 'click', function(){
			openWinCenter('新增客户类型','<c:url value='/products/systemConfect.do?method=onMainView' />'+'&type=03', 400,500,true);
		 });
	//--------------------------------------新增市场类型---------
	 Ext.EventManager.addListener(Ext.get('ext_config_marketType_set'), 'click', function(){
			openWinCenter('新增市场类型','<c:url value='/products/systemConfect.do?method=onMainView' />'+'&type=04', 400,500,true);
		 });
		 
	Ext.EventManager.addListener(Ext.get('ext_b_Look'), 'click', function(){//修改
	...
	Ext.EventManager.addListener(Ext.get('ext_b_change'), 'click', function(){//查看客户指标
             ...
             var url = '<c:url value="/client/client.do?method=onEditView"/>' +  '&clientId=' + guid;
	...
	Ext.EventManager.addListener(Ext.get('ext_b_delete'), 'click', function(){//删除数据
	      ...
	      if(num >= 1){
	      	  Ext.MessageBox.confirm("系统提示",'您确定要删除所经销商吗？',function(btnId){
					if(btnId == 'yes'){
					var str = "";
					var strName="";
				    for(var i= 0 ; i < num ; i ++){
				    ...
				    var str1 = Ext.util.Format.substr(strName,0,strName.length - 1);
						Ext.Ajax.request({//AJAX删除
								...
								success : function(response,options){
									...
									if(result.success){
										Ext.Msg.alert('系统提示','删除经销商信息成功');
										refresh();
								    }else{
										Ext.Msg.alert('系统提示','经销商信息被关联不能删除！');
									
								    }
								},
								failure : function(response,options){
									checkAjaxStatus(response);
									Ext.Msg.alert('系统提示','删除经销商数据信息失败');
								}
						});
					}
			 });
