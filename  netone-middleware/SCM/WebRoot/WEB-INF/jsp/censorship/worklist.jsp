...
<script src="<%=path%>/script/jquery-plugin/json2.js"
	type="text/javascript"></script>
<script language="javascript" type="text/javascript"
	src="<%=path%>/My97DatePicker/WdatePicker.js" charset="gb2312"></script>
<!-- 通过样式 按钮 -->
<link rel="stylesheet" type="text/css"
	href="<%=path%>/script/theme/main/common.css">
<style>
<!--
A {
	color: #006699;
...
<script type="text/javascript"> 
		...
		$(function() {
			...
			if (oTable == null) { //仅第一次检索时初始化Datatable
				...
				oTable = $('#_datatables').dataTable( {
					...
					"sAjaxSource": "<c:url value='/censorship/censorship.do?method=worklist' />",//获取数据的url
		...
		function retrieveData( sSource, aoData, fnCallback ) {
			...
			aoData.push( { "name": "status", "value": '${param.status}' } );
	...
	<body>
		...
		<div id="searchCondition"
			...
			<c:forEach var="columns" items="${queryColumn}" varStatus="step">
				<input type="radio" value="${columns.index}" id="querycolumn"
					name="querycolumn"
					<c:if test="${step.first}">checked="checked"</c:if>
					<c:if test="${columns.time==true}">onclick="$changeinput(0)"</c:if>
					<c:if test="${columns.time==false}">onclick="$changeinput(1)"</c:if> />${columns.name}
				</c:forEach>
			<!-- 
			...
			<input type="text" id="condition"
				style="font-size: 12px; font-weight: normal;" />
			<span id="datecondition" style="display: none;"> <input
					name="starttime" id="starttime" type="text"
					onchange="$inputCondition()" value="${beginTime}"
					style="width: 70px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'});" />
				到 <input name="endtime" id="endtime" type="text"
					onchange="$inputCondition()" value="${endTime}"
					style="width: 70px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'});" />
			</span>
