...
public class TurnpageFormUseView implements PageEvent {
	DyFormDesignService dys = null;

	DyFormService dysc = null;

	public TurnpageFormUseView() {
		if (dys == null) {
			try {
	...
	public Object page(Page page, Object obj) {
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			String viewItem = "<a href='/dyForm/data/showdata/display.do?lsh="
					+ object.getLsh() + "&formcode=" + object.getFormcode()
					+ "' target='_blank'><font color='blue'>[明细]</font></a>";
			//处理子表单链接
			String buff = SubFormViewMode.linkViewMode(search.getSubForm(),
					object);

			object.setColumn30(viewItem + "&nbsp;" + buff);
