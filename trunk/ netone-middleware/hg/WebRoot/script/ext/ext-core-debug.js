...
//it is a bug in this version. fireDocReady() be executed more then once, when docReadyEvent fired and some event function do "alert()", then docReadyEvent fired again before run "clearListeners()".
//change by guig
            if(docReadyProcId){
...
//VVVVVVVVVVVV add by guig  // replace the unname function in onWindowResize.
...
El.prototype = {
    ...
    replaceWith: function(el){
        ...
        El.uncache(this.id);
//VVVVVVVVVVVVV change by guig //in ie, the "parentNode.removeChild" make the old "this.dom" orphan, so use "Ext.removeNode" to remove it.
        Ext.removeNode(this.dom);
//^^^^^^^^^^^^^ change by guig
        this.dom = el;
        this.id = Ext.id(el);
...
El.garbageCollect = function(cleanFly){//add clean _flyweights  //changed by guig
...
//VVVVVV add by guig
    if (cleanFly === true) {//add clean _flyweights
        El._flyweights = {};
    }
//^^^^^ add by guig
}
