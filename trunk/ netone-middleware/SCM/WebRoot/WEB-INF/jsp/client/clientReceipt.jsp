		...
		<div id='container' style="width: 100%; height: 100%;">
			<div id="queryDiv"
				...
				<form id="queryForm" action="">
					...
					<input type="hidden" id="clientLoginName" name="clientLoginName"
						value="${code}">
					<input type="hidden" id="servicestation" name="servicestation" value="">
					<input type="hidden" id="operate" name="operate" value="">
				   <input type="hidden" id="status" name="status" value="3">
				    <input type="hidden" id="type" name="type" value="client">
				     <input type="hidden" id="tdNumber" name="tdNumber" value="">
...
<script type="text/javascript">
		 ...
		 initComponent: function() {
			...
			var config = {
				...
				columns:[
						...
						{header: "配送单号", width: 100, dataIndex: 'allocateCargoCode', sortable: true,hidden:true},
						{header: "TD号", width: 100, dataIndex: 'tdNumber', sortable: true},
						{header: "客户编号", width: 100, dataIndex: 'clientInfo.clientLoginName', sortable: true,hidden:true},
						{header: "客户名称", width: 80, dataIndex: 'clientInfo.clientName', sortable: true,hidden:true},
						{header: "联系人", width: 80, dataIndex: 'clientInfo.linkman', sortable: true,hidden: true,hidden:true},
						{header: "联系电话", width: 100, dataIndex: 'clientInfo.telphone', sortable: true,hidden:true},
						{header: "手机", width: 100, dataIndex: 'clientInfo.mobile', sortable: true,hidden:true},
						{header: "电子邮件", width: 100, dataIndex: 'clientInfo.email', sortable: true,hidden:true},
						{header: "送达站", width: 100, dataIndex: 'clientInfo.servicestation', sortable: true,hidden:true},
						{header: "送货地址", width: 100, dataIndex: 'clientInfo.deliverGoodsAddress', sortable: true,hidden:true},
						{header: "制单人", width: 100, dataIndex: 'operate', sortable: true,
						    renderer:
							function todo(value, cellmeta, record, rowIndex, columnIndex, store){
								 var str = record.get('operate');
								 var arrayList=str.split("[");
								 return arrayList[0];
							}},
						{header: "制单时间", width: 80, dataIndex: 'operateTime', sortable: true},
						{header: "状态", width: 120, dataIndex: 'status', sortable: true,hidden: false,renderer:
							function todo(value, cellmeta, record, rowIndex, columnIndex, store){
	      ...
	      this.store.on('beforeload', function(store,options){
				  var condition = {//取得HTML页面的查询条件
						...
						type:Ext.get('type').getValue(),
						tdNumber:Ext.get('tdNumber').getValue()
				  }; 
				  options=options||{}; 
  ...
  AllcoateCargo.Layout.Viewport =  Ext.extend(Ext.Viewport, {
   ...
   Ext.onReady(function(){//页面加载时候触发事件
    ...
    AllcoateCargoGrid2.addListener('celldblclick',function(grid, rowIndex, columnIndex, e){//列表双击事件--修改
	 var selectionSet = AllcoateCargoGrid2.getSelectionModel().getSelections(); 
           ...
           var status = selectionSet[0].data.status;
           var url = '<c:url value="/storage/allocateCargo.do?method=onLookForm"/>'+'&allocateCargoId=' + guid+'&status='+status;
           openWinCenter('查看配货单明细',url, 1120,800,true);
});
	
	 Ext.EventManager.addListener(Ext.get('ext_b_look'), 'click', function(){//查看配货单明细
			 var selectionSet = AllcoateCargoGrid2.getSelectionModel().getSelections(); 
			 if( selectionSet == undefined || selectionSet.length > 1 || selectionSet.length <=0){
				Ext.MessageBox.alert('系统提示','请选择一条记录进行查看!');
				return ;
			 }	
             var guid = selectionSet[0].data.allocateCargoId;
              var status = selectionSet[0].data.status;
             var url = '<c:url value="/storage/allocateCargo.do?method=onLookForm"/>'+'&allocateCargoId=' + guid+'&status='+status;
             openWinCenter('查看收货确认明细',url, 1120,800,true);
	});
});
</script>
