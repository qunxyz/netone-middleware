...
//objecttype,name,naturalname,active,description,appid,parentdir,actionurl,ou,extendattribute,inclusion,reference,aggre
...
<html>
	<head>
		...
		function nodeAction(id,ou,naturalname,parentdir,url){
			...
			var ajax = xmlhttp("servlet/AjaxServiceSvl?class=<%=className%>&id="+id+"&selectvalue=<%
...
=selectvalue%>");
 ...
 var subflowTypeCheck=document.forms[0].choiceSync;
                                    if(subflowTypeCheck.checked){

      	subflowTypeCheck='syncer';
      }else{
      	subflowTypeCheck='disyncer';
      }
                                submitSubflow(option.text,option.value,subflowTypeCheck);
		...
		<form action="" method="post">
			<table width="100%" height="100%" border="0" cellpadding="0"
				cellspacing="0" id="table">
						...
						&nbsp;&nbsp;<font color='red'><strong>同步子流程</strong></font>
...
value="${app.id}" ${app.naturalname==appnaturalname?"selected":"" }>
													${app.name }
												</option>
											</c:forEach>
										</select>
...
WebFXLoadTree("${root.name}","XMLFuncTreeSvl?parentid=${root.id}","javascript:nodeAction

('${root.id}','${root.ou}','${root.naturalname}','${root.parentdir}','${root.actionurl}')");
						  					functree.write();
						  					functree.expand();
								...
								<td align="center" valign="top">
									<select size="13" name="users" style="width: 
