...
public class SubProcessActorImp implements SubProcessActor {
	...
	public TWfRuntime executes(TWfWorklist curWorklist) {
		...
		try {
			processid = runtimemonitor.loadRuntime(curWorklist.getRuntimeid())
					.getProcessid();
	...
	private void syncRelevantvar(TWfRuntime runtimeSub,
		...
		try {
			for (int i = 0; i < dataFather.length; i++) {
				String dataname = dataFather[i].getName();
				DataField subObject = procsub.getDataField(dataname);
				_log.debug("check sub data:"+dataname);
				if (subObject != null) {// 检查子流程是否存在与父流程雷同的变量
					_log.debug("check sub data exist:"+dataname);
					TWfRelevantvar revSub = runtimemonitor.fetchRelevantVar(
							subflowRuntimeid, dataname);
					TWfRelevantvar revFather = runtimemonitor.fetchRelevantVar(
							fatherflowRuntimeId, dataname);
