...
public class XHtmlCachepool {
	...
	public static TCmsInfocell fetchPage(String cellid,
			HttpServletRequest request) {
		if (cellid == null) {
			return null;
		}
		String cachekey = WebCache._CACHE_ITEM_WEBCELL + cellid;
		if (WebCache.containCache(cachekey)) {// 存在缓存
		...
		if(!CellInfo._IN_TIME.equals(cellObj.getIntime())){
			WebCache.setCache(WebCache._CACHE_ITEM_WEBCELL + cellid, cellObj,
					dateCycle);
		}
