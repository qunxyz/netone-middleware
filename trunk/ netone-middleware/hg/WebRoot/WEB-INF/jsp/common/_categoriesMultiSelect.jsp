		 ...
		 initComponent: function() {
			...
			var config = {
				...
				store : new Ext.data.Store({
							url : "<c:url value='/products/products.do?method=onFindCategoriesSetById' />", // JSON数据
				...
				columns : [index,
					cb, 
					{header: "编码", align:'center',dataIndex: 'categoriesCode',sortable : true,
						renderer : function (value, metadata, record, rowIndex, columnIndex, store) {   
						    //build the qtip:   
						    var title = '&nbsp;' + record.get('categoriesCode') +   
						        ' ' + record.get('categoriesName');   
						    var tip = '';
					...
					{header: "名称", align:'center',dataIndex: 'categoriesName',sortable : true}
				],
							...
							var l = "";
						     ...
						     if (len<=0) {return;}
						      for ( var g = 0; g < len; g++) {
						       ...
						       l += f.getAt(g).data.categoriesId;
						       j += f.getAt(g).data.categoriesCode;
						       k += f.getAt(g).data.categoriesName;
						      }
						     _select(l,j,k);
						}},
...
function add_all(){//添加
	...
	for ( var p = 0; p < x.length; p++) {
	 var q = x[p].data.categoriesId;
	 var v = x[p].data.categoriesCode;
	 var k = x[p].data.categoriesName;
	 var s = false;
	 for ( var o = 0; o < u.getCount(); o++) {
	  if (u.getAt(o).data.categoriesId == q) {
	   s = true;
	   break;
	  }
	 }
...
/**
* 父窗口方法 _multSelectCategories(ids,values,texts) 
*/
function _select(ids,values,texts){
	if (window.opener && window.opener.location){
		opener._multSelectCategories(ids,texts);window.close();
	}else{
		alert('找不到父窗口!');
	}
}
