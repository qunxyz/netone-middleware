...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String className = "oe.security3a.web.human.AjaxHumanSelect";
%>
...
<html>
	<head>
		...
		function nodeAction(id,ou,naturalname,parentdir,url){
			...
			var ajax = xmlhttp("AjaxServiceSvl?class=<%=className%>&id="+id);
			...
			if(resp){
				...
				for(var i=0 ; i<users.length ; i++){
					if(users[i]){
						var user = new Map();
						user.load(users[i]);
						usersel.add(new Option(user.get("name"),user.get("description")));
					}
				}
			}
		}
		...
		function doSelect(){
			...
			if(result.length == 0){
				...
				opener.addSelectedUser3(result.options);
				window.close();
			}			
		}
