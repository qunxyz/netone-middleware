...
<HTML dir="LTR" lang="zh-CN">
	<HEAD>
		...
		function nextsubmit(){
			...
			var topvalue = document.getElementsByName("topvalue");
			for(var i=0;i<chkid.length;i++){
				if(chkid[i].checked){
					tgid_sql =chkid[i].value.split(":")[0];
					tgid_sql_Group =chkid[i].value.split(":")[1];
					top_sql=top[i].value;
					topvalue_sql=topvalue[i].value;
					if(topvalue_sql==""){
						alert("最值不能为空！");
	    				return false;
					}
					if(!isNaN(topvalue_sql)){
	    				alert("最值必须为数字！");
	    				return false;
	    			}
					if("topn"==top_sql){
						sqlStr += " order by "+tgid_sql+" DESC:"+tgid_sql_Group+",";
					} else if("botn"==top_sql){
						sqlStr += " order by "+tgid_sql+" ASC:"+tgid_sql_Group+",";
					}
	    			allchkid += chkid[i].value+",";
	    			alltop += top_sql+",";
					alltopvalue += topvalue_sql+",";
				}
			}
			document.all.allchkid.value=allchkid;
			document.all.alltop.value=alltop;
			document.all.alltopvalue.value=alltopvalue;
			...
			document.all.allorder.value="";
   			...
   			document.all.tgfiltvalue.value=alltopvalue;
   			document.additem.submit();
		}
		//取消脚本
	...
	<BODY bgcolor="#FFFFFF">
		<FORM ACTION="<%=path%>/NextAnalyse.do?task=Done" METHOD="POST" name="additem">
							...
							<tr class="td-02">
								...
								<td>
									...
									<span style="width:18px;border:0px solid red;"> 
										<select name="r00" style="margin-left:-100px;width:118px; "
											onChange="this.parentNode.previousSibling.value=this.value">
			...
			<script type="text/javascript">
				...
				var alltop = document.all.alltop.value;
				var alltopvalue = document.all.alltopvalue.value;
				if(allchkid!=""){
					allchkid = allchkid.split(",");
					alltop = alltop.split(",");
					alltopvalue = alltopvalue.split(",");
					for(var i=0;i<allchkid.length-1;i++){
						for(var j=0;j<chkid.length;j++){
							if(allchkid[i]==chkid[j].value){
								chkid[j].checked=true;
								top[j].value=alltop[i];
								topvalue[j].value=alltopvalue[i];
							}
						}
					}
				}
