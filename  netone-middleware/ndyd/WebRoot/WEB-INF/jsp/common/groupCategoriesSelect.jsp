...
<html>
	<head>
		...
		<script type="text/javascript" src="<%=basePath%>/script/ext/extend/TreeCheckNode.js"></script>
...
　　　				autoScroll:true,
					...
					bbar:new Ext.Toolbar([
						 {text : '确认',id:'ext_b_selectRow',handler:function(){
						 	var selectText = getSelectNodesAttributes('text');
						 	if (selectText!=''){
						 		opener._mselect(getSelectNodesAttributes('id'),getSelectNodesAttributes('text'));
								window.close();
						 	} else {
						 		alert('请选择一个或多个组!');
						 	}
				     ...
				     baseAttrs: {uiProvider: Ext.ux.TreeCheckNodeUI } //添加 uiProvider 属性   
						...
						click : function(node,e){
						 ...
						 dblclick : function(node,e){
						   if(node.attributes.id != 'root'){
						     opener._select(node.attributes.id,node.attributes.text);
						     window.close();
						   }
						 }
					}
...
Ext.onReady(function(){
    ...
    Ext.getCmp('treepanel').bbar.dom.align = 'center';//按钮居中
