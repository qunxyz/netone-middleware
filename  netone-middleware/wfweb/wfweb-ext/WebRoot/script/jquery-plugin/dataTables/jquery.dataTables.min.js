...
var _oExt=$.fn.dataTableExt;_oExt.sVersion="1.6.0";_oExt.iApiIndex=0;_oExt.oApi={};
...
}return parseInt(sThis)>=parseInt(sThat)};this.fnSort=function(aaSort){var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);
...
oSettings.aoData.push({nTr:this,_iId:oSettings.iNextId++,_aData:[],_anHidden:[],_sRowStripe:""});
...
}var nTrs=$("tbody:eq(0)>tr",oSettings.nTable);for(i=0,iLen=nTrs.length;i<iLen;i++){nTrs[i].parentNode.removeChild(nTrs[i])
}var nBody=$("tbody:eq(0)",oSettings.nTable);if(nBody[0]){for(i=0,iLen=anRows.length;
...
}},0)}if(typeof fnCallback=="function"){fnCallback(oSettings)}})}function _fnSortingClasses(oSettings){var i,j,iFound;
...
var _that=this;return this.each(function(){var i=0,iLen,j,jLen;for(i=0,iLen=_aoSettings.length;
i<iLen;i++){if(_aoSettings[i].nTable==this){alert("DataTables warning: Unable to re-initialise DataTable. Please use the API to make any configuration changes required.");
...
}for(i=0,iLen=oSettings.aaSorting.length;i<iLen;i++){if(typeof oSettings.aaSorting[i][2]=="undefined"){oSettings.aaSorting[i][2]=0
}var oColumn=oSettings.aoColumns[oSettings.aaSorting[i][0]];for(j=0,jLen=oColumn.asSorting.length;
j<jLen;j++){if(oSettings.aaSorting[i][1]==oColumn.asSorting[j]){oSettings.aaSorting[i][2]=j;
