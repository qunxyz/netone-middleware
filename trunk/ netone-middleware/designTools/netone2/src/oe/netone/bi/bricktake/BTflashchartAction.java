 ...
 *  
 */
public class BTflashchartAction extends HttpServlet {
...
//		data.setYAxisLabel(yString);//
		...
		String json = service.entry(ChartCompVar.COMP_LINE_LINK_CHART, data);
		// 3.输出参数
		service.output(request, json, data.getTitle());
		}
		if(OGXML.getTubiaotype().equals("zhutu")){
			// 2.指定输出报表格式 具体变量查看com.report.flashchart.entity.ChartCompVar.java
			String json = service.entry(ChartCompVar.COMP_BAR_LINK_CHART, data);
			// 3.输出参数
			service.output(request, json, data.getTitle());
		}
