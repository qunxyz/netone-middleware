		...
		<div region="north"
			...
			<div id="app-header">
				<div id="top" sytle="height:52px;width:1000px;background-image: url('<%=cssURL%>/top_nd.gif');">
					<div id="topFontInfo">公司应用-集团终端审批流程</div>
				</div>
		...
		<c:choose>
			<c:when test="${param.query=='look'}">
				<iframe id="flowMainFrame" name="flowMainFrame" 
					src="<c:url value='/groupTerminal/groupTerminal.do?method=onQuery&groupTerminalId=${param.groupTerminalId}&runtimeid=${param.runtimeid}&operatemode=${param.operatemode}&readonly=${param.readonly}' />"
			...
			<c:otherwise>
				<iframe id="flowMainFrame" name="flowMainFrame" 
					src="<c:url value='/groupTerminal/groupTerminal.do?method=onEditView&groupTerminalId=${param.groupTerminalId}&workcode=${param.workcode}&runtimeid=${param.runtimeid}&operatemode=${param.operatemode}' />"
					scrolling="auto" frameborder="0" 
					style="width: 1000px; height: 100%; margin: 0 auto;">
				</iframe>
			</c:otherwise>
