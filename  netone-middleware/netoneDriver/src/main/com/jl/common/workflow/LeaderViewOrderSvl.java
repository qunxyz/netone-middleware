...
public class LeaderViewOrderSvl extends HttpServlet {
	...
	private List menTop5(String deptid) {
		if (!WebCache.containCache("menTop5")) {
			...
			WebCache.setCache("menTop5", list, dateinfo);
		}
		return (List) WebCache.getCache("menTop5");

	}

	private List deptTop5(String deptid) {

		if (!WebCache.containCache("deptTop5")) {
		...
		return (List) WebCache.getCache("deptTop5");

	}

	private List flowTop5(String deptid) {
		if (!WebCache.containCache("flowTop5")) {
		...
		for (int i = 1; i <= 5 && i <= procMa.length; i++) {
			...
			for (Iterator iterator = procecmax_tmp.keySet().iterator(); iterator
					.hasNext();) {
		...
		long time = System.currentTimeMillis() + 1800000L;// 30分钟缓存
		Date dateinfo = new Date(time);
		WebCache.setCache("flowTop5", procemaxFinal, dateinfo);
		}
		return (List)WebCache.getCache("flowTop5");
	}
