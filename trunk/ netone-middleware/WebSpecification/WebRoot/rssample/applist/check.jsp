	...
	<body style="font-size: 12px;margin: 22px">
		...
		<div style="width: 100%;height: 100%">
			<form action="ApplistModifySvl?task=editsave" method="post"
					...
					<tr>
						...
						<td>
							<select name='color'
								...
								<option value='black'
									<c:if test="${app.extendattribute=='black'}">selected</c:if>>
									黑色
								</option>
								<option value='red'
									<c:if test="${app.extendattribute=='red'}">selected</c:if>>
									红色
								</option>
								<option value='blue'
									<c:if test="${app.extendattribute=='blue'}">selected</c:if>>
									蓝色
								</option>
								<option value='green'
									<c:if test="${app.extendattribute=='green'}">selected</c:if>>
									绿色
								</option>
								<option value='white'
									<c:if test="${app.extendattribute=='white'}">selected</c:if>>
									白色
								</option>
							</select>
				...
				<br>
				<div align="center">
					<input type="submit" name="btnnew" value="确 定" class="butt">
