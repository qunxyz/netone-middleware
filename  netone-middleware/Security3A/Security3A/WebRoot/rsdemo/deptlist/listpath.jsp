	...
	<head>
		...
		<script type="text/javascript">
			...
			function humaninput(id){
				window.open("<%=path%>/rsinfo/dept/DepartmentInput.do?task=addfile&id="+id,"create",'width=500,height=100,resizable=yes,left=150,status=yes');
			}
			//人员导出
			function humanoutput(id){
				form1.action = "<%=path%>/rsinfo/dept/DepartmentOutput.do?id="+id;
				form1.submit();
			}
			//关联用户
			function relatinghuman(id){
				window.open("<%=path%>/rsinfo/dept/DepartmentRelate.do?&id="+id,'关联用户','width=300,height=300,resizable=yes,left=300,top=180,status=yes');
			}
			//创建人员
			function humanadd(){
				window.open("<%=path%>/humanAdd.do?task=add","useradd","width=700,height=350,resizable=yes,left=150,status=yes");
			}
			//修改人员
			function humanmodify(value){
				window.open("<%=path%>/humanModify.do?task=modify&extendattribute="+value,"usermod","width=700,height=350,resizable=yes,left=150,status=yes");
			}
			//删除人员
			function humandel(value){
				form1.action="<%=path%>/humanDel.do?extendattribute="+value;
				form1.submit();
			}
			//复制人员
			function humancopy(value){
				window.open("<%=path%>/humanCopy.do?task=copy&description="+value,"usercopy","width=700,height=350,resizable=yes,left=150,status=yes");
			}
		</script>
	</head>
				...
				<table width="96%" border="0" align="center" cellpadding="5"
					...
					<tr>
						<td colspan='9' align='right' nowrap>
							&nbsp;&nbsp;
							<input type="button" value="新建人员" onclick="humanadd();"
								class="butt">
							&nbsp;&nbsp;
							...
							&nbsp;&nbsp;
							<input type="button" name="btnin" value="导 入"
								onclick="humaninput();" class="butt">
							&nbsp;&nbsp;
							<input type="button" name="btnout" value="导 出"
								onclick="humanoutput();" class="butt">
							&nbsp;&nbsp;
							<input type="button" name="" value="关联用户"
								onclick="relatinghuman();" class="butt">
						</td>
					</tr>
				</table>
				<br>
						...
						<tr>
							...
							<td nowrap>
								<c:if test="${list.inclusion == '1'}">
									...
									<a href="javascript:humaninput('${list.id}');">导入</a>
									<a href="javascript:humanoutput('${list.id}');">导出</a>
									<a href="javascript:relatinghuman('${list.id}');">关联用户</a>
								</c:if>
								<c:if test="${list.inclusion != '1'}">
									<a href="javascript:humancopy('${list.id}');">复制</a>
									<a href="javascript:humanmodify('${list.id}');">修改</a>
									<a href="javascript:humandel('${list.id}');">删除</a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
