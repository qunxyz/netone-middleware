...
<html>
	<head>
		...
		<script>var onuser='<rs:logininfo />';</script>
		<!-- 样式文件 -->	
		${linkcss}
		<!-- 时间控件脚本 -->
		${datecompFunc}
...
$.fn.autogrow = function(options) {
  ...
  return this;
	...
	$(function() {
		...
		$("#column4").removeAttr("onfocus");
		$("#column5").removeAttr("onfocus");
		$("#column7").removeAttr("onfocus");
		
		
		$("#column4").focus(function(){
			WdatePicker({maxDate:'#F{$dp.$D(\'column5\')}'})
		})
		
		$("#column5").focus(function(){
			WdatePicker({minDate:'#F{$dp.$D(\'column4\')}'})
		})
		
		$("#column7").focus(function(){
			WdatePicker({minDate:'#F{$dp.$D(\'column5\')}'})
		})
		
		//终端名称
		$('#column8').autocomplete('<c:url value="/Soasvl?datatype=json&naturalname=SOASCRIPT.SOASCRIPT.HG.CLIENTBYEMP"/>', {
				multiple: true,
				dataType: "json",
				autoFill: false,
				mustMatch: true,
				...
				extraParams:{
			       sr_empid: function() { return onuser; }   
			    }, 
