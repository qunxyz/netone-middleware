...
import oe.netone.bi.pa.Allocation;
import oe.netone.bi.pa.Devise;
import oe.netone.bi.pa.XMLAnalysis;
import oe.netone.bi.pa.XMLDevise;
...
public class ShowDate {
		...
		try {
			...
			for (int i = 0; i < act.length; i++) {
				if (act[i].getImplementation() != null) {// 空节点不需要进入
					...
					upobj.setOu(String.valueOf(act.length));
					lisxt.add(upobj);
				}
			}
	...
	// portal 的数据
	public String portalQuery(String naturalname) {
		ResourceRmi resourceRmi = null;
		try {
			resourceRmi = (ResourceRmi) RmiEntry.iv("resource");
		} catch (MalformedURLException e) {
	...
	// 页的数据
	public String pageQuery(String naturalname) {
		ResourceRmi resourceRmi = null;
		try {
			resourceRmi = (ResourceRmi) RmiEntry.iv("resource");
		} catch (MalformedURLException e) {
