			...
			<table WIDTH="100%" class="main_nd" CELLPADDING="0" CELLSPACING="0">
				...
				<tr style="background-image: url('<%=path%>/FDL/Dubang/Image/index_2323.gif'); width: 100%;color: #a52526;padding-top: 5px; font-weight: bold; height: 25px;"> 
					<td COLSPAN=11 class="label_file" >
						输入文件标题:
						<span style="vertical-align: middle;">
							<input type="text" id="filename" name="filename" style="width: 320px;" readonly="readonly"/>
						</span>
						<span style="vertical-align: middle;">
...
<script>
	...
	function $upload(){//上传
		...
		var msgTip = Ext.MessageBox.show({
	  	...
	  	Ext.Ajax.request({
				url:"<c:url value='/file.do?method=onUploadFrameFile'/>&id="+d_unid+"&filename="+filename+"&f_type="+f_type,//请求的服务器地址
				form:'form1',//指定要提交的表单id
				method:'POST',
				sync: true,
				success : function(response,options){
