...
public class DepartmentServiceImpl extends BaseService implements
	...
	public void exportDepartmentInfo(HttpServletRequest request,
		...
		String code = request.getParameter("code");
		String list = request.getParameter("list");
		String type = request.getParameter("ex_type") == null ? "" : request
				.getParameter("ex_type").trim();
		Map conditionMap = new HashMap();
		if (StringUtils.isNotEmpty(level)) {
			conditionMap.put("level", level.trim());
		}
		...
		try {
			...
			if (type.equals("dArea")) {
				groupReport.format(format, "公司地图信息",
						getReport_map(conditionMap), response);
			} else {
				groupReport.format(format, "客户信息", getReport(conditionMap),
						response);
			}
		} catch (Exception e) {
			log.error("导出信息出错!", e);
	...
	private Report getReport(Map conditionMap) throws Exception {
		Collection result = commonDAO.select(
				"Department.queryDepartmentDetailInfoForPage", conditionMap);
		String[] dataHeaderSet = { "departmentCode", "departmentName",
				"parentDepartmentName", "levelName" };
		String[] headerSet = { "编码", "名称", "隶属", "业务类型" };
		Map<String, Integer> position = new HashMap();
		ReportExt reportExt = new ReportExt();
		Report report = reportExt.getReportList(result, dataHeaderSet,
				headerSet, position);
		reportExt.setSimpleTitleFooter(report, "客户信息");
		return report;
	}

	private Report getReport_map(Map conditionMap) throws Exception {
		Collection result = commonDAO.select(
				"Department.queryDepartmentDetailInfoForPage", conditionMap);
		String[] dataHeaderSet = { "departmentId", "departmentCode",
				"departmentName", "nLevel", "nLevelName", "levelName", "map" };
		String[] headerSet = { "编号", "编码", "名称", "树级别", "隶属关系", "业务类型", "地图信息" };
		Map<String, Integer> position = new HashMap();
		ReportExt reportExt = new ReportExt();
		Report report = reportExt.getReportList(result, dataHeaderSet,
				headerSet, position);
		reportExt.setSimpleTitleFooter(report, "组织机构地图信息");
		return report;
	}
