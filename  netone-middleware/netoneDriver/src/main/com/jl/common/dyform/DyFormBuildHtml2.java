...
public final class DyFormBuildHtml2 {
	...
	public static final int AvailWidth = 800;// 可用显示宽度
	...
	public static final String TableExtProperties = " style=\"word-break:break-all\" bgcolor=\"white\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\"";
	...
	protected static String routeAppointComp(String htmltype, String id,
		...
		if (arr[0][0].equals(htmltype)) {// 00:通用
			...
			return DyFormComp.getText(id, userinfo_, style, classname, true,
					extvalue);
		} else if (arr[18][0].equals(htmltype)) {// 25:当前部门
			String userinfo_ = "";
			if (StringUtils.isNotEmpty(userinfo)) {
			...
			return DyFormComp.getText(id, userinfo_, style, classname, true,
					extvalue);
		} else if (arr[19][0].equals(htmltype)) {// 26:多选选项
			return DyFormComp.getCheckboxs(id, value, style, classname,
					readonly, selectedvalue, extvalue);
		} else if (arr[20][0].equals(htmltype)) {// 27:组织机构单选
			return DyFormComp.getSelect_(id, value, style, classname, readonly,
					single_select + "&type=" + arr[20][0] + "&naturalname="
							+ selectedvalue, extvalue);
		} else if (arr[21][0].equals(htmltype)) {// 28:组织机构多选
			return DyFormComp.getSelect_(id, value, style, classname, readonly,
					multi_select + "&type=" + arr[21][0] + "&naturalname="
							+ selectedvalue, extvalue);
		} else if (arr[22][0].equals(htmltype)) {// 29:URL
	...
	protected static boolean checkSelect_(String htmltype) {
		String[][] arr = DyFormConsoleIfc._HTML_LIST;
		if (arr[11][0].equals(htmltype)) {// 17:单选资源
			return true;
		} else if (arr[12][0].equals(htmltype)) {// 18:多选资源
			return true;
		} else if (arr[13][0].equals(htmltype)) {// 20:PORTAL项
						...
						if (columnmap.get(field) != null) {
							...
							if (hidden != null && hidden == false) {
								...
								if (isedit == false) {
									...
									// style = "width:80%;";
									// }
									String style = "width:" + column.getWidth()
											+ "px;";
	...
	public static String buildMainForm(DyForm dyform, boolean isedit,
						...
						if (columnid.equals(field)) {
							if (columnmapx.get(field) != null) {
								...
								if (hidden__ != null && hidden__ == false) {
									...
									availColumnWidth = calcAvailColumnWidth(yoffset_
											.intValue());
		...
		String TableExtPropertiesExt = " border=\"1\" width=\"900px\" style=\"word-break:break-all\" bgcolor=\"#FFFFDB\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\"";
	...
	public static String buildForm(DyForm dyform, boolean isedit,
		...
		String TableExtPropertiesExt = " border=\"1\" width=\"900px\" style=\"word-break:break-all\" bgcolor=\"#D2CF98\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\"";
		String html_ = DyFormComp.getTable(formcode, html.toString(),
				TableStyle, TABLE_FORM, 0, TableExtPropertiesExt);
		// 输出文档普通字段内容结束
	...
	public static String buildSubForm(DyForm subdyform, String fatherlsh,
		...
		for (int i = 0; i < _formx.length; i++) {
			...
			if (hidden == false) {
				...
				String style = "width:" + _qc1.getWidth() + "px;";
				if (_qc1.getWidth() <= 0) {
					style = "width:100px;";
				}

				th.append(DyFormComp.getTh("", "&nbsp;" + columnname + musktip,
						TableTdStyle2 + style, TABLE_TD_HEADER,
						" align='left' "));
				++colspan;
...
//		}
		...
		btnstr.append("function " + onclickAddFunctionname + "(){" + xhtml
		...
		String html_ = DyFormComp.getTable(formcode, html.toString(),
				"display:block;word-wrap:break-all;overflow-x:auto;", "", 0,
				TableExtProperties);
		String html_btn = "";
		if (isedit) {
		...
		htmlall.append(eventListenScripts.toString() + "<div id='tabs" + uid
				+ "' >" + html_ + html_btn + "</div>");

		String functionscript = "$('tabs"
				+ uid
				+ "').tabs();$('#"
				+ formcode
				+ "').dataTable({'sScrollY':'100%','sScrollX':'100%','sScrollXInner':'100%','bPaginate': false,'bLengthChange': true,'bFilter': false,'bSort': false,'bInfo': false,'bAutoWidth': false });";

		htmlall.append(DyFormComp.getTag(
