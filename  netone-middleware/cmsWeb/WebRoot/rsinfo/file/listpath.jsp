...
<html>
	<head>
		...
		<script type="text/javascript">
		//单选择 需要根据应用的需要来订制
		function checkthis(id,name,naturalname) {
			opener.fetchFile(id,name,naturalname);
			window.close();
		}
		//多选择 需要根据应用的需要来订制
		function checkthese(){
			...
			<form action="" method="post" name="form1">
				...
				<table width="96%" border="0" align="center" cellpadding="5"
					...
					<tr>
						...
						<td colspan='7'>
							<input type="button" value="创建文件目录" onclick="newInclusion();"
								class="butt">
							&nbsp;&nbsp;
					...
					<tr>
						<td colspan='9' align='right' nowrap>
							...
							&nbsp;&nbsp;
							<input type="button" value="上传文件" onclick="newElementBinary();"
								class="butt">
							&nbsp;&nbsp;
				...
				<table id="hang_table" width="96%" border="0" align="center"
					...
					<tr>
						...
						<td class="td_titt_bg" width="100" nowrap>
							中文名称
						</td>
						<td class="td_titt_bg" width="100" nowrap>
							预览
						...
						<tr>
							...
							<td nowrap>
								<c:if test="${list.inclusion == '1'}">
									<a href="javascript:inclusionlink('${list.id}','${list.ou}');">[${list.name}]</a>
								</c:if>
								<c:if test="${list.inclusion != '1'}">
								${list.name}
								</c:if>
							...
							<td nowrap>
								...
								<c:if test="${list.inclusion != '1'}">
									<a href="DownloadSvl?fileid=${list.id}">下载 </a>
								</c:if>
							</td>
							<td nowrap>
								${list.created}
							...
							<td nowrap>
								...
								<a href="javascript:checkthis('${list.id}','${list.name}','${list.naturalname}');">选择</a>
							</td>
						</tr>
					</c:forEach>
				</table>
