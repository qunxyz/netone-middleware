...
<html>
	<head>
		...
		<script type="text/javascript">
		//单选择 需要根据应用的需要来订制
		function checkthis(name,natrualname) {
		if(opener.selectthis==undefined){
			alert('非选择状态');
			return;
		}
			opener.selectthis(name,natrualname);
			window.close();
		}
		//多选择 需要根据应用的需要来订制
		function checkthese(){
...
<a href='javascript:newInclusion();'><font color='blue'>新建目录</font></a>
					...
					<tr>
						<td colspan='9' align='right' nowrap>
							&nbsp;&nbsp;
							<input type="button" value="新建样式" onclick="newElemnt();"
								class="butt">
					...
					<c:forEach items="${list}" var="list">
						<tr>
							...
							<td nowrap>
								...
								<a href="javascript:checkthis('${list.name}','${list.naturalname}');">选择</a>
							</td>
						</tr>
					</c:forEach>
				</table>
