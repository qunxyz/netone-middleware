...
<script>
   function onSaveButton(){
	  ...
	  if(importFileObj.value == ''){
	      Ext.MessageBox.alert('系统提示','请选择要导入的经销商信息文件！');
		 return  ;
	  }
	  var arrayList=importFileObj.value.split(".");
	   if(arrayList[1]!='xls'){
	    ...
	    Ext.Ajax.request({
					url :'<c:url value="/client/import.do?method=onImportClientInfoFile"/>',//请求的服务器地址
   ...
   function dowanLoad(){
       openWinCenter('下载','<c:url value="/client/import.do?method=onDownLoadClientFile"/>&type=client', 500,250,true);
   } 
