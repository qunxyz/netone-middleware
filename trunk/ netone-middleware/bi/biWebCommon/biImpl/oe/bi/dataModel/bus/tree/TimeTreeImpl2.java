 ...
 * 
 */
public class TimeTreeImpl2 implements TimeTree, BiTree2 {
	...
	public String fetchYearInfo(String datamodelid, String columnid) {
		...
		for (int i = 0; i < yeardims.length; i++) {
			String preInfo = MakeXMLTree2.makeXMLTimeInfo(datamodelid,
	...
	public String fetchHour(String datamodelid, String columnid, String day) {
		...
		for (int i = 0; i < hourInfo.length; i++) {
			String preInfo = MakeXMLTree2.makeXMLTimeInfo(datamodelid, day
	...
	public String fetchMin(String datamodelid, String columnid, String hour) {
		...
		for (int i = 0; i < minInfo.length; i++) {
			String preInfo = MakeXMLTree2.makeXMLTimeInfo(datamodelid, hour
					+ ":" + minInfo[i][0], minInfo[i][1], "5");
			buf.append(preInfo);
		}
		return HEAD_XML + buf.toString() + END_XML;
	}
	...
	public String fetchSec(String datamodelid, String columnid, String min) {
		...
		for (int i = 0; i < secInfo.length; i++) {
			String preInfo = MakeXMLTree2.makeXMLTimeInfo(datamodelid, min
	...
	public String fetchTimeInfo(String datamodelid, String columnid,
			String nodevalue2, String level) {
		if (columnid == null) {
			return HEAD_XML + END_XML;
		}
		if (ActionDigTree._ROOT_NODE.equals(columnid)) {
			return this.fetchYearInfo(datamodelid, columnid);
		} else if (columnid.matches(_RULE_YEAR)) {
			return this.fetchMonthInfo(datamodelid, columnid, columnid);
		} else if (columnid.matches(_RULE_MONTH)) {
			return this.fetchDayInfo(datamodelid, columnid, columnid);
		} else if (columnid.matches(_RULE_DAY)) {
			return this.fetchHour(datamodelid, columnid, columnid);
		} else if (columnid.matches(_RULE_HOUR)) {
			return this.fetchMin(datamodelid, columnid, columnid);
		} else if (columnid.matches(_RULE_MIN)) {
			return this.fetchSec(datamodelid, columnid, columnid);
		}
