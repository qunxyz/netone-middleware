...
<HTML>
	<HEAD>
		...
		function success_do_changeTgList(xmlhttp){
			...
			var html = "";
			html += "<input type='checkbox' name='columnchkid' value='name_en'>隶属{string}&nbsp;&nbsp;&nbsp;&nbsp;";
			html += "链接路径:<input type='text' name='columntxt' value=''>"
			html += "<select name='columnsel'>"
			html += "<option value=''></option>";
			html += "<option value='name_en'>隶属{string}</option>";
			html += "<option value='start_time'>时间{date}</option>";
			for(var j=0 ; j<keyarr.length ; j++){
				html += "<option value="+keyarr[j].substring(0,keyarr[j].indexOf("{"))+">"+map.get(keyarr[j])+"</option>";
			}
			html += "</select><br>"
			
			html += "<input type='checkbox' name='columnchkid' value='start_time'>时间{date}&nbsp;&nbsp;&nbsp;&nbsp;";
			html += "链接路径:<input type='text' name='columntxt' value=''>"
			html += "<select name='columnsel'>"
			html += "<option value=''></option>";
			html += "<option value='name_en'>隶属{string}</option>";
	...
	<BODY bgcolor="#FFFFFF">
		<FORM ACTION="" METHOD="POST" name="additem">
							...
							<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%">
								...
								<TR>
									<TD ALIGN="LEFT">
										...
										&nbsp;&nbsp;&nbsp;
										<FONT CLASS="OecInlineInfo">第 10 步, 共 10 步&nbsp;&nbsp;</FONT>
			...
			<DIV
				...
				<hr>
				<br>
				<FONT CLASS="OecInstructionText">帮助提示: 为了丰富报表的信息，可以通过该步骤的配置给相关的报表数据加上链接，以丰富的报表数据展现。</FONT>
				<!-- End--Title----功能标题与提示---->
				<!-- Start--Body---- 功能内容定义  --->
				<table>
					<tr>
