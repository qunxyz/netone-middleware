 ...
 * </ul>
 * 
 * @version $Id: ConnectorServlet.java,v 1.1 2012/04/24 01:02:49 chenjx Exp $
 */
		...
		if (!RequestCycleHandler.isEnabledForFileBrowsing(request))
			...
			if (!currentDir.exists())
				...
				if (command.equals(CommandHandler.GET_FOLDERS))
					...
					newStr = new String(newStr.getBytes("iso8859-1"),"UTF-8");//对中文名称进行重新编码
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!RequestCycleHandler.isEnabledForFileUpload(request))
			...
			if (!currentDir.exists())
				...
				upload.setHeaderEncoding("UTF-8");//解决文件名是中文
				...
				try {
					...
					filename = UUID.randomUUID().toString()+"."+extension;//解决中文名图片的显示
