...
public class SocketHttpRequester {
    ...
    public boolean post(String path, Map<String, String> params, FormFile[] files) throws Exception{     
        ...
        for(FormFile uploadFile : files){//得到文件类型数据的总长度
            ...
            if(uploadFile.getInStream()!=null){
            	fileSize = uploadFile.getFile().length();
        ...
        for(FormFile uploadFile : files){
            StringBuilder fileEntity = new StringBuilder();
             ...
             Log.i("getParameterName()", uploadFile.getParameterName());
        ...
        String rsStr = reader.readLine();
        System.out.println("返回数据："+rsStr);

        if(rsStr.indexOf("200") < 0){//读取web服务器返回的数据，判断请求码是否为200，如果不是200，代表请求失败
     ...
     * @param file 上传文件
     */
    public boolean post(String path, Map<String, String> params, FormFile file, Handler inhandler) throws Exception{
    ...
    private void sendMsg(int flag)
	{
	    ...
	    if(flag == 0){
	    	...
	    	bundle.putLong("uploadFileSize", uploadFileSize);
	    }else if(flag == 2){
