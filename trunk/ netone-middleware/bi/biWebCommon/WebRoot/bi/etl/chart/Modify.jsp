...
<HTML dir="LTR" lang="zh-CN">
	<HEAD>
		...
		function forward(){
			...
			document.additem.action="<%=path%>/NextChart.do?task=Next";
	...
	<BODY bgcolor="#FFFFFF">
		<FORM ACTION="<%=path%>/NextChart.do?task=Finish" METHOD="POST"
			name="additem">
							...
							<TABLE WIDTH="100%" BORDER="0" CELLPADDING="1" CELLSPACING="0">
								<TR CLASS="OecBgColorDark">
									...
									<TD WIDTH="100%" valign="center">
										<FONT CLASS="OecGlobalPageTitle">图表定义</FONT>
									</TD>
			...
			<DIV
				...
				<hr>
				<br>
				<FONT CLASS="OecInstructionText">帮助提示: 选择图表展现相关的明细参数</FONT>
				<!-- End --Title----功能标题与提示---->
				<BR>
				<!-- Start--Body---- 功能内容定义  --->
				<div
						...
						<tr class="td-02">
							...
							<td bgcolor="#FFFFFF" align="left" nowrap>
								<c:forEach items="${dimvalues}" var="dm">
									<c:if test="${dm[1]=='类型'}">
									${dm[1]}
										<select id="${dm[0]}" name="seldim" <c:if test="${isDynamic=='is'}">disabled="disabled"</c:if>>
											<OPTION value="-1">
												未选择
											</OPTION>
									...
									<c:if test="${dm[1]=='时间'}">
										${dm[1]}
										<select id="${dm[0]}" name="seldim" <c:if test="${show!='show'}">style="display:none"</c:if>>
											<OPTION value="-1">
												未选择
											</OPTION>
											<c:forTokens items="${dm[2]}" delims="," var="dmv">
						...
						<tr class="td-02">
							...
							<td>
								...
								<input type="text" value="${fo.piccolor }" name="piccolor"
									style="width: 80">
								&nbsp; X轴文字倾斜值(0~1)
								<input type="text" value="${fo.xqingxie }" name="xqingxie" style="width: 50">
							</td>
						</tr>
					</table>
				</div>
