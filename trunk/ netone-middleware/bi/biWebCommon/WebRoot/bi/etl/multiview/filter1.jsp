...
<HTML dir="LTR" lang="zh-CN">
	<HEAD>
		...
		function nextsubmit(){
			...
			var txtalarm = document.getElementsByName("txtalarm");
			for(var i=0;i<chkid.length;i++){
				if(chkid[i].checked){
					var type = txtalarm[i].title;
					var before = chkid[i].value;
					tgid_sql =before.split(":")[0];
					tgid_sql_Group =before.split(":")[1];
					alarm_sql=alarm[i].value;
					txtalarm_sql=txtalarm[i].value;
					if(txtalarm_sql==""){
						alert("门限值不能为空！");
	    				return false;
					}
					if(type=="number" &&!isDigit(txtalarm_sql)){
	    				alert("门限值必须为数字！");
	    				return false;
	    			}
	    			tgid_sql = tgid_sql.substring(tgid_sql.indexOf("{")+1,tgid_sql.indexOf("}"));
					if("like"==alarm_sql){
	    				sqlStr +=" and "+tgid_sql+" like '$"+txtalarm_sql+"$'";
	    			} else {
	    				sqlStr +=" and "+tgid_sql+alarm_sql+txtalarm_sql;
	    			}
	    			allchkid += before+",";
	    			allalarm += alarm_sql+",";
					alltxtalarm += txtalarm_sql+",";
				}
			}
			document.all.allchkid.value=allchkid;
			document.all.alltop.value="";
			document.all.alltopvalue.value="";
			document.all.allalarm.value=allalarm;
			document.all.alltxtalarm.value=alltxtalarm;
			document.all.allorder.value="";
   			document.all.tgfiltSqlvalue.value=sqlStr;
   			document.all.tgfiltvalue.value="";
   			var sql = " and 1=1";
   			document.all.sqlview.value = sql + document.all.tgfiltSqlvalue.value;
	...
	<BODY bgcolor="#FFFFFF">
		<FORM ACTION="<%=path%>/NextFilter.do?task=filter2" METHOD="POST"
			name="additem">
							...
							<tr class="td-02">
								<td>
									<input type="checkbox" name="chkid"
										value="${tg}">
									${tgnamelist[vs.index]}&nbsp;&nbsp;&nbsp;${tgtypelist[vs.index]}
								</td>
								<td>
									门限值：
									<c:if test="${tgtypelist[vs.index]=='string'}">
										<select name="alarm">
											<c:forEach items="${_CHOICE_ALARM2}" var="ca">
												<option value="${ca[0]}">
													${ca[1]}
												</option>
											</c:forEach>
									...
									<c:if test="${tgtypelist[vs.index]=='number'}">
										<select name="alarm">
											<c:forEach items="${_CHOICE_ALARM3}" var="ca">
												<option value="${ca[0]}">
													${ca[1]}&nbsp;&nbsp;
												</option>
											</c:forEach>
									...
									<input type="text" name="txtalarm" value="" title="${tgtypelist[vs.index]}" style="width: 70px">
								</td>
							</tr>
						</c:forEach>
			...
			<script type="text/javascript">
				...
				var allalarm = document.all.allalarm.value;
				var alltxtalarm = document.all.alltxtalarm.value;
				if(allchkid!=""){
					allchkid = allchkid.split(",");
					allalarm = allalarm.split(",");
					alltxtalarm = alltxtalarm.split(",");
					for(var i=0;i<allchkid.length-1;i++){
						for(var j=0;j<chkid.length;j++){
							if(allchkid[i]==chkid[j].value){
								chkid[j].checked=true;
								alarm[j].value=allalarm[i];
								txtalarm[j].value=alltxtalarm[i];
							}
						}
					}
				}
