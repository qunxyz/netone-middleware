...
import oe.security3a.Certificate;
...
public class CAOnlineUserMgr implements OnlineUserMgr {
	...
	// * 初始化保存对应的key映射 dxl
	// */
	// private static void saveKeyToMap() {
	// String caKey = "";
	// try {
	// CupmRmi cupmRmi = (CupmRmi) RmiEntry.iv("cupm");
	// caKey = cupmRmi.fetchConfig("CAMAP");
	// } catch (Exception e) {
	// // TODO Auto-generated catch block
	// e.printStackTrace();
	// }
	// if (StringUtils.isNotEmpty(caKey)) {
	// String[] keyList = StringUtils.split(caKey, ";");// 获得所有的CaKey列表
	// if (keyList != null && keyList.length > 0) {// 判断分割后数组是否为空
	// for (int j = 0; j < keyList.length; j++) {
	// String[] key = StringUtils.split(keyList[j], ":");
	// if (key != null && key.length == 2) {
	// CA_KEY_MAP.put(key[0], key[1]);
	// }
	// }
	// }
	// }
