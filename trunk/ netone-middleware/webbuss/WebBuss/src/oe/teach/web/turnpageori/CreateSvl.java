...
public class CreateSvl extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("done".equals(ope)) {
			...
			String estprice = request.getParameter("estprice");
			String types = request.getParameter("types");
			types = WebStr.encode(request, types);
			String description = request.getParameter("description");
			description = WebStr.encode(request, description);
			// 增加参与者字段 by chen.jia.xun 2009-09-30
			String participant = request.getParameter("participant");
			participant = WebStr.encode(request, participant);

			MyAccount ac = new MyAccount();
			ac.setName(name);
			ac.setOriprice(Double.parseDouble(oriprice));
			ac.setCurprice(Double.parseDouble(curprice));
			ac.setEstprice(Double.parseDouble(estprice));
			ac.setTypes(types);
			ac.setDescription(description);
			ac.setLsh(UUID.randomUUID().toString().replace("-", ""));
			ac.setParticipant(participant);
			long time = System.currentTimeMillis();
			ac.setCreated((new Timestamp(time)).toString());
			MyAccountDao dao = new MyAccuntDaoImpl();
			dao.create(ac);
			WebTip.htmlInfo("添加成功", true, true, response);
		} else {
			// 打开创建页面
